...
class ItemCardTokenised extends Component
{
    public $order;
    public $product;
    public $quantity;
    ...
    {
        return view('livewire.item-card-tokenised');
    }
    ...
    public function addProductToOrder($qty = 1)
    {
        ...
        if ($this->validate()) {
            ...
            $order = $this->order;
            $quantity = $qty ? $qty : $this->quantity;
            ...
            if ($existingItem = $order->items()->where('product_id', $product->id)->first()) {
                ...
                $validatedData['product_rate'] = $product->rate;
