...
const restructured = redumped.reduce((acc, obj) => {
    if (!acc[obj["table_name"]]) {
        acc[obj["table_name"]] = [];
        //"";
    }
    acc[obj.table_name].push(obj["column_name"]);
    //+= obj.column_name + ",";
...
let newResults = {
    product_shop: ["id", "shop_id", "product_id", "created_at", "updated_at"],
    reports: ["id", "created_at", "updated_at"],
    password_resets: ["email", "token", "created_at"],
    settings: ["id", "key", "value", "created_at", "updated_at", "user_id"],
    customers: [
        "id",
        "name",
        "membership_number",
