...
<script>
    ...
    let sxdata = {
        ...
        getSelectedItems(dropdown) {
            // console.log(this.$refs[dropdown.name] ? this.$refs[dropdown.name].selected : 'No ref for ' + dropdown.name);
            return this.$refs[dropdown.name] ?
                dropdown.items.filter(item => this.$refs[dropdown.name].selected.includes(String(item.value))) : [];
        }
    };
    // console.log(sxdata);
</script>
<div class="border-0 border-green-500 flex w-max rounded-lg text-base" x-data="sxdata">
    <!-- Filters container -->
    <div class="md:flex md:flex-wrap md:border-4 border-cyan-400 items-start gap-2 border-0">
        <template x-for="dropdown in dropdowns" :key="dropdown.name">
            <div x-data="{
            ...
            }" class="relative border-0 w-content p-0 m-0 border-purple-500"
                :x-ref="dropdown.name">
                ...
                <button type="button" @click="open = !open; $nextTick(() => { if(open) $refs.searchInput?.focus() })"
                    class="inline-flex justify-between w-full rounded md:w-auto px-3 py-2 text-base text-stone-700 bg-gray-50 border border-stone-300 hover:border-stone-400 appearance-none focus:outline-none focus:ring-2 focus:ring-rose-200 focus:border-rose-500">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    class="absolute z-10 w-full mt-1 rounded-md shadow-md md:min-w-48 bg-white ring-1 ring-black ring-opacity-5"
                    style="display: none;">
                    <!-- Search input with clear button -->
                    <div class="relative px-2 ">
                        <input x-model="search" x-ref="searchInput" @focus="$el.select()"
                            class="block w-full pl-3 pr-8 py-2 text-gray-800 rounded-md border border-gray-300 focus:outline-none focus:ring-indigo-500 shadow-inner focus:border-indigo-500"
                        ...
                        <button type="button" @click="search = ''" class="absolute inset-y-0 right-3 flex items-center"
                            ...
                            <svg class="h-5 w-5 text-gray-400 hover:text-indigo-600" fill="currentColor"
                                viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                    ...
                    <div class="py-1 max-h-60 overflow-y-auto ">
                        <template x-for="item in filteredItems" :key="item.value">
                            <div @click.stop="selected.includes(String(item.value)) ? selected = selected.filter(i => i !== String(item.value)) : selected.push(String(item.value))"
                                class="flex items-center px-3 py-2 text-gray-700 hover:bg-indigo-100 hover:text-indigo-800 cursor-pointer"
                                :class="{ 'bg-indigo-100': selected.includes(String(item.value)) }">
    ...
    {{-- <div class="mt-3 p-3 border-4 border-slate-300 rounded-lg">
                ...
                <div>
                    <template x-for="item in getSelectedItems(dropdown)" :key="item.value">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm bg-rose-100 text-rose-800">
                            <span x-text="item.label">eee</span>
                            <button type="button"
                                @click="$refs[dropdown.name].selected = $refs[dropdown.name].selected.filter(i => i !== String(item.value))"
