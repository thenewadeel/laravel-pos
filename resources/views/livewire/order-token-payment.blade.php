<div class="mx-1 px-1">
    ...
    <div class="card" wire:loading.class="bg-red-200">
        {{$err}}
        <div class="font-bold text-center text-lg p-0 m-2">
            <div class=" flex flex-row items-center">
                {{-- {{ __('order.Payment') }} --}}
                {{-- <i class="fas fa-cog text-md mx-2" onclick="toggleSettingsBar()"></i>

                <script>
                    function toggleSettingsBar() {
                        document.getElementById('settings_bar').classList.toggle('hidden')
                    }
                </script> --}}
        ...
        <div class=" p-0 m-0 flex flex-col items-center">
            {{-- <div class="flex flex-col items-center w-full hidden m-2 rounded-md border border-slate-500 justify-between"
                id="settings_bar">
                <div class="flex flex-row items-center w-full m-2 rounded-md justify-end">
                    <i class="fas fa-print text-md mx-2"></i>
                    <input type="text" name="printer_ip" wire:model="printer_ip"
                        class="mx-2 shadow-inner shadow-slate-500 rounded-md md:max-w-48 max-w-24 p-0 m-0  text-center font-bolder text-md self-center" />
                </div>
                <div class="flex flex-row items-center w-full m-2 rounded-md justify-end">
                    Chit Title
                    <input type="text" name="print_title" wire:model="print_title"
                        class="mx-2 shadow-inner shadow-slate-500 rounded-md md:max-w-48 max-w-24 p-0 m-0  text-center font-bolder text-md self-center" />
                </div>
                <div class="flex flex-row items-center w-full m-2 rounded-md justify-end">
                    Chit Subtitle
                    <input type="text" name="print_subtitle" wire:model="print_subtitle"
                        class="mx-2 shadow-inner shadow-slate-500 rounded-md md:max-w-48 max-w-24 p-0 m-0  text-center font-bolder text-md self-center" />
                </div>
            </div> --}}
